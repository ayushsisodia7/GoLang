...
func main() {
	...
	select { //Here’s a non-blocking receive.
	case msg := <-messages: //If a value is available on messages then select will take the <-messages case with that value.
	...
	msg := "hi"
	select { //A non-blocking send works similarly.
